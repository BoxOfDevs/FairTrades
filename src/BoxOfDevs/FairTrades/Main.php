	...
	switch($this->trade_part[$sender->getName()]) {
		...
		if(isset($args[1])) {
			...
			if($this->trade_part[$player2->getName()] === 1 and $this->trade_with[$player2->getName()] === $sender and $args[0] === "accept") { //If the trade is accepted
				...
				$sender->sendMessage("You accepted the trade! You have 45 seconds to talk together about the trade");
		...
		if(isset($args[2])) {
			...
			$sender->sendMessage("Usage: /trade additem <item:damage> <count>,  /trade removeitem <item:damage> <count> or /trade finish");
		}
		break;
        case 5.5:
        case 5:
        ...
        switch($args[0]) {
            ...
            if($this->trade_part[$player2->getName()] = 5.5) {
                $player2->sendMessage($sender->getName(). " accepted the trade  !");
                $sender->sendMessage("You accepted the trade.");
                $player2->sendMessage("You have both accepted the trade. Processing transfer...");
                $sender->sendMessage("You have both accepted the trade. Processing transfer...");
                $this->items[$sender->getName()]->transferItems($player2);
                $this->items[$player2->getName()]->transferItems($sender);
                $player2->sendMessage("Trade done !");
                $sender->sendMessage("Trade done !");
            ...
            unset($this->items[$player2->getName()]);
                ...
                $player2->sendMessage($sender->getName(). " accepted the trade. ");
                $sender->sendMessage("You accepted the trade. You can cancel this while the other player haven't accepted by doing /trade decline");
            }
            break;
            case "decline":
            $player2->sendMessage($sender->getName(). " declined the trade. All the trade has been cancelled.");
            $sender->sendMessage("You declined the trade. All the trade has been cancelled.");
            unset($this->trade_part[$sender->getName()]);
            unset($this->trade_part[$player2->getName()]);
            unset($this->trade_witht[$sender->getName()]);
            unset($this->trade_with[$player2->getName()]);
